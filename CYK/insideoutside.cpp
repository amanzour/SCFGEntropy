...
int  insideoutside::cykchoice (int v, int i, int j, int r)
{
	int reval;
	if ((i >= 0)&&(i <= j)&&(j < winwidth))
		//reval = alphamatrix.at(winbeg+i).at(j-i);
		reval = cykmatrix.at(v).at(i).at(j).at(r);
	else
        cout<<"wrong cky "<<i<<" "<<j<<" "<<r<<endl;
	return reval;
}
...
void insideoutside::alphatable(const string &winstr)
{
	...
	vector< vector< vector<int> > > cykt;
	vector< vector<int> > cykvalue;   //for initialize vector of cykt
	vector<int> basic_ele_cyk;
	//clearalpha();

	basic_ele_cyk.push_back(0);
	basic_ele_cyk.push_back(0);
	basic_ele_cyk.push_back(0);
	winlength = winstr.length();
	for (i=0; i<winlength; i++)
        cykvalue.push_back(basic_ele_cyk);
	if (flagalpha == 0)
	{
		value.resize(maxwidth);
		//cykvalue.resize(maxwidth);
		for (i=0; i<winlength; i++)
		{
			alphat.push_back(value);
			cykt.push_back(cykvalue);
		}
		for (i=0; i< nontnum; i++)
		{
			alphamatrix.push_back(alphat);
			cykmatrix.push_back(cykt);
		}
...
void insideoutside::cal_alpha(int v, int i, int j, const string &oristr)
{
	...
	int r, k, choice;
	ruleinfo currule;
	vector<scidouble> curVal;
    vector< vector<int> > cyk_posi; //position for cyk
    //vector< vector< vector<int> > > arr_cyk_posi; // array of position for cyk
    vector<int> basic_ele_cyk;
	...
	{
	    curalpha = scidouble(0);
		currule = grammar.at(map_alpha.at(v).at(r));
        basic_ele_cyk.at(0) = -1;
		basic_ele_cyk.at(1) = -1;
        basic_ele_cyk.at(2) = r;
		if ((currule.ruletype == 1)&&(i==j)) //X:a
		{
            curalpha = scidouble(currule.prob) * qprob(i, oristr);
            curVal.push_back(curalpha);
            basic_ele_cyk.at(0) = i;
            basic_ele_cyk.at(1) = -1;
            cyk_posi.push_back(basic_ele_cyk);
		}
		if ((currule.ruletype == 2)&&(i<j)) //X:aH
        {
            curalpha = scidouble(currule.prob) * qprob(i, oristr) * alpha(currule.non1, i+1, j);
            curVal.push_back(curalpha);
            basic_ele_cyk.at(0) = i;
            basic_ele_cyk.at(1) = -1;
            cyk_posi.push_back(basic_ele_cyk);
        ...
        {
			curalpha = scidouble(currule.prob) * basepair(i,j,oristr) * alpha(currule.non1, i+1, j-1);
            curVal.push_back(curalpha);
            basic_ele_cyk.at(0) = i;
            basic_ele_cyk.at(1) = j;
            cyk_posi.push_back(basic_ele_cyk);
        }
		if ((currule.ruletype == 4)&&(j>=(i+mindist+1))) //X:aHbY
			for (k=i+1; k<=j-1; k++)
			{
				curalpha = scidouble(currule.prob)
							* basepair(i,k,oristr)
							* alpha(currule.non1, i+1, k-1)
							* alpha(currule.non2, k+1, j);
                curVal.push_back(curalpha);
                basic_ele_cyk.at(0) = i;
                basic_ele_cyk.at(1) = k;
                cyk_posi.push_back(basic_ele_cyk);
			}
		if ((currule.ruletype == 5)&&(i<j)) //X:Ha
		{
			curalpha = scidouble(currule.prob) * qprob(j, oristr) * alpha(currule.non1, i, j-1);
            curVal.push_back(curalpha);
            basic_ele_cyk.at(0) = -1;
            basic_ele_cyk.at(1) = j;
            cyk_posi.push_back(basic_ele_cyk);
		...
		if ((currule.ruletype == 6)&&(j>=(i+mindist+1))) //X:HaYb
			for (k=i+1; k<=j-1; k++)
			{
                ...
                curVal.push_back(curalpha);
                basic_ele_cyk.at(0) = k;
                basic_ele_cyk.at(1) = j;
                cyk_posi.push_back(basic_ele_cyk);
			}
        if ((basic_ele_cyk.at(0) == -1) && (basic_ele_cyk.at(1) == -1))
        {
            curVal.push_back(curalpha);
            cyk_posi.push_back(basic_ele_cyk);
        }
	}
	curalpha = curVal.at(0);
    choice = 0;
	for (r=1; r<(int)curVal.size(); r++)
        if (curVal.at(r) > curalpha )
        {
            curalpha = curVal.at(r);
            choice = r;
        }
	alphamatrix.at(v).at(i).at(j)     = curalpha;
	cykmatrix.at(v).at(i).at(j).at(0) = cyk_posi.at(choice).at(0);
	cykmatrix.at(v).at(i).at(j).at(1) = cyk_posi.at(choice).at(1);
	cykmatrix.at(v).at(i).at(j).at(2) = cyk_posi.at(choice).at(2);
}
