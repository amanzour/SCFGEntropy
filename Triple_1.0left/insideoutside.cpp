...
scidouble insideoutside::alpha_left(int v, int i, int j)
{
	scidouble reval;
	if ((i >= 0)&&(i <= j)&&(j < winwidth))
		//reval = alphamatrix.at(winbeg+i).at(j-i);
		reval = alphamatrix_left.at(v).at(i).at(j);
	else
		if (i>j)
			reval.AssignDouble(1);
		else
...
scidouble insideoutside::beta_left(int v, int i, int j)
{
	  if ((v==0) && (i < 0) && (j > winwidth - 1)) 
		  return scidouble(1);
	  if ((v>=1) && (i < 0) && (j > winwidth - 1)) 
	  ...
	  if ((i==-1)&&(j<=winwidth-1))
			return betamatrix_left.at(v).at(winwidth).at(j);//!!!
	  ...
	  */
	  ...
	  return betamatrix_left.at(v).at(i).at(j);
}
...
void insideoutside::alphatable(const string &winstr)
{
	...
	if (flagalpha == 0)
	{
		...
		for (i=0; i< nontnum; i++)
			alphamatrix_left.push_back(alphat);
...
void insideoutside::cal_alpha(int v, int i, int j, const string &oristr)
{
	...
	for (r=0; r<(int)map_alpha.at(v).size(); r++)
	{
		...
		if ((currule.ruletype == 1)&&(i==j)) //X:a
			curalpha_left = curalpha_left + scidouble(currule.prob) * qprob(i, oristr);

		if ((currule.ruletype == 2)&&(i<j)) //X:aH
			if (currule.non1!=v)
				curalpha_left = curalpha_left + scidouble(currule.prob) * qprob(i, oristr) * alpha(currule.non1, i+1, j);
			else
		...
		if ((currule.ruletype == 3)&&(j>=(i+mindist+1))) //X:aHb
			curalpha_left = curalpha_left + scidouble(currule.prob) * basepair(i,j,oristr) * alpha(currule.non1, i+1, j-1);

		if ((currule.ruletype == 4)&&(j>=(i+mindist+1))) //X:aHbY
			if (currule.non2!=v)
				for (k=i+1; k<=j-1; k++)
					curalpha_left = curalpha_left + scidouble(currule.prob) 
								* basepair(i,k,oristr) 
		...
		if ((currule.ruletype == 5)&&(i<j)) //X:Ha
			if (currule.non1!=v)
				curalpha_left = curalpha_left + scidouble(currule.prob) * qprob(j, oristr) * alpha(currule.non1, i, j-1);
			else
				curalpha_left = curalpha_left + scidouble(currule.prob) * qprob(j, oristr) * alpha_left(currule.non1, i, j-1);
		...
		if ((currule.ruletype == 6)&&(j>=(i+mindist+1))) //X:HaYb
			if (currule.non1!=v)
				...
				for (k=i+1; k<=j-1; k++)
					curalpha_left = curalpha_left + scidouble(currule.prob)
								* basepair(k, j, oristr)
								* alpha_left(currule.non1, i, k-1)
								* alpha(currule.non2, k+1, j-1);

	}
	alphamatrix.at(v).at(i).at(j) = curalpha + curalpha_left;
	alphamatrix_left.at(v).at(i).at(j) = curalpha_left;
}
...
void insideoutside::cal_beta(int v, int i, int j, const string &winstr)
{
	...
	for (r=0; r<(int)map_beta.at(v).size(); r++)
	{
		currule = grammar.at(map_beta.at(v).at(r));
			...
			if (currule.ruletype == 2) // v':av
				if (currule.leftnon!=v)
					curbeta_left = curbeta_left + scidouble(currule.prob) * qprob(i, winstr) * beta(currule.leftnon, i-1, j);
				else
					curbeta_left = curbeta_left + scidouble(currule.prob) * qprob(i, winstr) * beta_left(currule.leftnon, i-1, j);
			
			if ((currule.ruletype == 3) && (j>=(i+mindist+1))) // v':avb
			{
				curbeta_left = curbeta_left + scidouble(currule.prob) * basepair(i,j,winstr) * beta(currule.leftnon, i-1, j+1);
			}
				

			//if ((currule.ruletype == 4)&&(j>=(i+mindist+1)))//v':av_1bv_2
				...
				if (currule.non1 == v) // v_1 == v
				{
					if (currule.leftnon!=currule.non2)
						for (k=j+1; k<winwidth; k++)
							curbeta_left = curbeta_left + scidouble(currule.prob) * basepair(i,j,winstr)
										* beta(currule.leftnon,i-1,k+1)
										* alpha(currule.non2,j+1,k);
					else
						for (k=j+1; k<winwidth; k++)
							curbeta_left = curbeta_left + scidouble(currule.prob) * basepair(i,j,winstr)
										* beta_left(currule.leftnon,i-1,k+1)
										* alpha(currule.non2,j+1,k);
				}
				if (currule.non2 == v) // v_2 == v
				{
					if (currule.leftnon!=currule.non2)
						for (k=0; k<=(i-mindist-1); k++)
							curbeta_left = curbeta_left + scidouble(currule.prob)*basepair(k,i,winstr)
										*beta(currule.leftnon,k-1,j)
										*alpha(currule.non1,k+1,i-1);
					else
						for (k=0; k<=(i-mindist-1); k++)
							curbeta_left = curbeta_left + scidouble(currule.prob)*basepair(k,i,winstr)
										*beta_left(currule.leftnon,k-1,j)
										*alpha(currule.non1,k+1,i-1);
				}
			}
			...
			if (currule.ruletype == 5) // v':va
				if (currule.leftnon!=v)
					curbeta_left = curbeta_left + scidouble(currule.prob) * qprob(j, winstr) * beta(currule.leftnon,i, j+1);
				else
					curbeta = curbeta + scidouble(currule.prob) * qprob(j, winstr) * beta(currule.leftnon,i, j+1);

			//if ((currule.ruletype == 6)&&(j>=(i+mindist+1))) //v':v_1av_2b
				...
				if (currule.non1 == v) // v_1 == v
				{
					if (currule.leftnon!=currule.non1)
						for (k=j+mindist+1; k<winwidth; k++)
							curbeta_left = curbeta_left + scidouble(currule.prob)*basepair(j,k,winstr)
	...
	if (i==-1)
	{
	betamatrix.at(v).at(winwidth).at(j) = curbeta+curbeta_left;
	betamatrix_left.at(v).at(winwidth).at(j) = curbeta_left;
	}
	else
	{
	betamatrix.at(v).at(i).at(j) = curbeta+curbeta_left;
	betamatrix_left.at(v).at(i).at(j) = curbeta_left;
	}
}
...
void insideoutside::betaspace(int MaxWidth)
{
	...
	vector< vector<vector<scidouble> > > betat_left;
	winwidth = MaxWidth;
	value.resize((winwidth+1));
	for (i=0; i<=winwidth; i++)
	...
	for (i=0; i<nontnum; i++)
		betamatrix_left.push_back(betat);
}
